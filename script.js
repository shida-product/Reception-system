document.addEventListener('DOMContentLoaded', () => {
    ...
    function createTicketCard(ticket) {
        ...
        if (ticket.status === 'preparing') {
            ...
            // 準備完了：受付時間と完了時間を上下2段表示
            timeHtml = `<div class="ticket-time">受付時間 ${receptionTime}</div><div class="ticket-time">完了時間 ${completedTime}</div>`;
        }
        ...
        function getGmailLink() {
            if (ticket.notificationType === 'email' && ticket.url) {
                ...
                const body = `${ticket.name ? ticket.name + ' 様' : 'お客様'}\n\n` +
                    ...
                    `お待ちしております。`;
                return `https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent(ticket.url)}&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
            }
            return null;
        }
        ...
        // 通知リンク（email/LINEタイプの場合、歯車の左側に配置）
        let notificationLinkHtml = '';
        if (ticket.notificationType === 'email') {
            const gmailLink = getGmailLink();
            if (gmailLink) {
                notificationLinkHtml = `<a href="${gmailLink}" target="_blank" class="mail-link" title="完了通知を送る">完了通知を送る</a>`;
            } else {
                notificationLinkHtml = `<span class="mail-link disabled" title="メール設定が必要">完了通知を送る</span>`;
            }
        } else if (ticket.notificationType === 'line') {
            if (ticket.url) {
                notificationLinkHtml = `<a href="${ticket.url}" target="_blank" class="mail-link" title="LINEを開く">LINEを開く</a>`;
            } else {
                notificationLinkHtml = `<span class="mail-link disabled" title="LINE URL設定が必要">LINEを開く</span>`;
            }
        }
